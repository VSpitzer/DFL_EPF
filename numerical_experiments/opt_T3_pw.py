...
class LotSizingModel_pw(optModel):

    def __init__(self, T, N_t1, tab_t1, C, x_on, d, S, s_i):
        """
        Args:
            T (int): Time horizon
        ...
        self.x_on = x_on
        self.s_i = s_i
        self.tab_t1 = np.array(tab_t1, dtype=np.int32)
    ...
    def _getModel(self):
        ...
        handle = LoadLibrary("C:\\Users\\Victor\\Documents\\Code\\DFL_EPF\\DFL_toolbox\\lib_LS_CC_SUB_T3_pw.so")
        lib = CDLL(name="C:\\Users\\Victor\\Documents\\Code\\DFL_EPF\\DFL_toolbox\\lib_LS_CC_SUB_T3_pw.so",handle = handle)
    ...
    def solve(self):
        ...
        #solution
        self._model["lib"].CLSP_T3(self._model["sol"], self.N_t1, self.tab_t1, self.T, self.cost, self.s_i, self.S, self.d, self.C, self.x_on)
        
        #Optimal sol and objective
        sol = np.array(self._model["sol"])
