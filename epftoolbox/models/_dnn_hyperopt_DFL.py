...
from epftoolbox.models import DNNModel_DFL
...
def _build_space(nlayer, loss, data_augmentation, n_exogenous_inputs):
    ...
    space = {
        ...
        'c_reg': hp.choice('c_reg', [
            {'val': None, 'kappa': 0},
            {'val': 'proj', 'kappa': hp.uniform('kappa', 1, 1e5)}]),
        'scaleX': hp.choice('scaleX', ['No', 'Norm', 'Norm1', 'Std',
                                                   'Median', 'Invariant']),
        'scaleY': hp.choice('scaleY', ['No', 'Norm1'])        
    }
    
    if loss == 'DPO':
        space['c_reg'] = hp.choice('c_reg', [
            {'val': None, 'kappa': 0},
            {'val': 'proj', 'kappa': hp.uniform('kappa', 1, 1e5)}])
        space['lambd'] = hp.choice('lambd', [None])
        space['sigma'] = hp.uniform('sigma', 1e-5, 10)
        space['n_samples'] = hp.choice('n_samples', [4])
    if loss == 'DBB':
        space['c_reg'] = hp.choice('c_reg', [
            {'val': None, 'kappa': 0},
            {'val': 'proj', 'kappa': hp.uniform('kappa', 1, 1e5)}])
        space['lambd'] = hp.uniform('sigma', 1e-5, 100)
        space['sigma'] = hp.choice('sigma', [None])
        space['n_samples'] = hp.choice('n_samples', [None])
    if loss == 'SPO':
        space['c_reg'] = hp.choice('c_reg', [
            {'val': None, 'kappa': 0},
            {'val': 'proj', 'kappa': hp.uniform('kappa', 1, 1e5)}])
        ...
        space['n_samples'] = hp.choice('n_samples', [None])
    if loss == 'FY':
        space['c_reg'] = hp.choice('c_reg', [{'val': None, 'kappa': 0}])
        space['lambd'] = hp.choice('lambd', [None])
        space['sigma'] = hp.choice('sigma', [None])
        space['n_samples'] = hp.choice('n_samples', [None])
...
def _hyperopt_objective(hyperparameters, optmodel, loss, trials, trials_file_path, max_evals, nlayers, dfTrain, dfTest, 
    ...
    if trials.losses()[0] is not None:
        ...
        RegretVal = trials.best_trial['result']['Regret Val']
        RegretTest = trials.best_trial['result']['Regret Test']
        
        print('\n\nTested {}/{} iterations.'.format(len(trials.losses()) - 1,
              max_evals))

        print('Best Regret - Validation Dataset')            
        print("  Regret: {:.1f} %".format(RegretVal))
        print('\nBest Regret - Test Dataset')
        print("  Regret: {:.1f} %".format(RegretTest))
    ...
    if hyperparameters['scaleY'] in ['Norm1']:
    ...
    # Initialize model
    forecaster = DNNModel_DFL(neurons=neurons, n_features=Xtrain.shape[-1], optmodel=optmodel, 
                     dropout=hyperparameters['dropout'], batch_normalization=hyperparameters['batch_normalization'], c_reg=hyperparameters['c_reg'], 
                     lr=hyperparameters['lr'], n_samples=hyperparameters['n_samples'], sigma=hyperparameters['sigma'], lambd=hyperparameters['lambd'],
                     verbose=False, optimizer='adam', activation=hyperparameters['activation'],
                     epochs_early_stopping=20, scaler=scaler, loss=loss,
                     regularization=hyperparameters['reg']['val'], 
                     lambda_reg=hyperparameters['reg']['lambda'],
                     initializer=hyperparameters['init'])

    # build training dataloader
    dataset = pyepo.data.dataset.optDataset(optmodel, Xtrain, Ytrain)
    dataloader_train = DataLoader(dataset, batch_size=192, shuffle=True)
    # build validation dataloader
    dataset = pyepo.data.dataset.optDataset(optmodel, Xval, Yval)
    dataloader_val = DataLoader(dataset, batch_size=192, shuffle=True)
    
    forecaster.fit(dataloader_train, dataloader_val)
    ...
    regret_validation = np.mean(Regret(Yval, Yp, optmodel))
    ...
    RegretTest = np.mean(Regret(Ytest, Yp, optmodel)) 
    ...
    # dataset
    return_values = {'loss': regret_validation, 'Regret Val': regret_validation, 'Regret Test': RegretTest,
                     'status': STATUS_OK}
                          
    return return_values

def hyperparameter_optimizer_DFL(optmodel, loss, path_datasets_folder=os.path.join('.', 'datasets'), 
    ...
    trials_file_name = 'DNN_hyperparameters_nl' + str(nlayers) + '_dat' + str(dataset) + '_' + str(loss) + "loss" + \
    ...
    space = _build_space(nlayers, loss, data_augmentation, n_exogenous_inputs)


    # Perform hyperparameter optimization
    fmin_objective = partial(_hyperopt_objective, optmodel=optmodel, loss=loss, trials=trials, trials_file_path=trials_file_path, 
